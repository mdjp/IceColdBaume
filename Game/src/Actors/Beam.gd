...
func get_user_movement():
	var motorl = get_parent().get_node("MotorLeft")
	var motorr = get_parent().get_node("MotorRight")
	var res = [get_input("right"), get_input("left")]
	if(abs(res[0]) == 1):
		motorr.set_volume_db(linear2db(0.5))
	else:
		motorr.set_volume_db(linear2db(0))
	if(abs(res[1]) == 1):
		motorl.set_volume_db(linear2db(0.5))
	else:
		motorl.set_volume_db(linear2db(0))
	return res
